...
*/
      ...
      case FN_DIR: {                    /* \ffdir.() - Directory count. */
	      ...
	      debug(F100,"FN_DIR or FN_DIR","",0);
              xflags |= ZX_DIRONLY;
#ifdef OS2
              zxpn = 1;                 /* Use the alternate list */
#endif /* OS2 */
          } else {                      /* List only files */
	      debug(F100,"Not FN_DIR or FN_DIR","",0);
              xflags |= ZX_FILONLY;
#ifdef OS2
              zxpn = 1;                 /* Use the alternate list */
#endif /* OS2 */
...
*/
	...
	if (!*s) goto xemail;
	while (*s == SP || *s == HT) s++;
	if (ckindex("From: ",s,0,0,0) == 1) s += 6;
	if (ckindex("Sender: ",s,0,0,0) == 1) s += 8;
	while (*s == SP || *s == HT) s++;	
		...
		CHAR c = NUL; 
                ...
                if ((x == 's' || x == ':') && (k > 1)) { /* Substring wanted */
                    if (vnambuf[k-1] == ']') {
                        ...
                        for (i = k-1; i > 0; i--) {
                            if (vnambuf[i] == '[') {
				int j = i+1; /* Point past open bracket */
				/* Skip past any sign */
				if (vnambuf[j] == '-' || vnambuf[j] == '+')
				  j++;
				for (; j < k; j++) {
				    if (!isdigit(vnambuf[j])) {
					c = vnambuf[j];
					vnambuf[j] = NUL;
					x1 = atoi(&vnambuf[i+1]);
					vnambuf[j] = c;
