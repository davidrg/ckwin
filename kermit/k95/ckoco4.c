...
CopyVscrnToKbdBuffer( BYTE vmode, int select_mode ) {
    ...
    if ( isunicode() ) {
        ...
        if ( vmode == VTERM ) {
            ...
            for ( i=1; i<len; i++ ) {
                if ( Uselection[i-1] != XCR || Uselection[i] != LF ) {
        ...
        pData[0] = selection[0];
        for ( i=1, j=1 ; i<len ; i++ )
            if ( selection[i-1] != XCR || selection[i] != LF )
                pData[j++] = selection[i] ;
        pData[j] = '\0';
    ...
    if ( use_unicode ) {
        if ( pUClipbrdData ) {
            ...
            if ( vmode == VTERM ) {
                ...
                if ( nbytes > 0 )
                    bytecount = nbytes;
                for ( i=1; i<len; i++ ) {
                    if ( pUClipbrdData[i-1] != XCR || pUClipbrdData[i] != LF ) {
        ...
        if ( pClipbrdData ) {
            ...
            if ( pData != NULL ) {
                ...
                for ( i=1, j=1 ; i<len ; i++ )
                    if ( pClipbrdData[i-1] != XCR || pClipbrdData[i] != LF )
