...
if [ "$F_SOFT_FONT" = "1" ]; then
	...
	printf '\x1bP0;1;1;10;0;2;16;0{P????oo????/????NN????/????@@????;
	??[[??[[??/??????????/??????????;??__??__??/?r~~rr~~r?/??@@??@@??;
...
if [ "$F_VT420_FEATURES" = "1" ]; then
  ...
  if [ "$F_PAGING" = "1" ]; then
    ...
    # DECCRA that copies data from there. As turning on DECLRMM clears DHL
    # attributes, the top title will have to be rewritten after DECLRMM is
    # turned back off.
